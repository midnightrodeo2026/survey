...
(function () {
  ...
  function fromRecord(r) {
    return {
      character: r.character || "", discord: r.discord || "", mainCls: r.main && r.main.cls || "", mainSpec: r.main && r.main.spec || "", role: r.role || "", commitment: r.commitment || "",
      flexCls: r.flex && r.flex.cls || "", flexSpec: r.flex && r.flex.spec || "", offspec: r.offspec || "", interests: r.interests || [], roster: r.roster || "",
  ...
  $("#btn-csv").addEventListener("click", async () => {
    ...
    const lines = [cols.join(",")].concat(responses.map(r => [
      r.submittedAt, r.character, r.discord, clsBy(r.main.cls) && clsBy(r.main.cls).name, specBy(r.main.cls, r.main.spec) && specBy(r.main.cls, r.main.spec).name, r.role, labelOf(C.commitment, r.commitment),
      r.flex && clsBy(r.flex.cls) && clsBy(r.flex.cls).name || "", r.flex && specBy(r.flex.cls, r.flex.spec) && specBy(r.flex.cls, r.flex.spec).name || "", r.flex && r.flex.role || "", labelOf(C.offspec, r.offspec),
